...
{
	uint8 Local_u8ErrorState=OK;
	if ( (copy_PinNum<=DIO_PIN7) && (copy_PinNum>=DIO_PIN0) )
	{
		if (copy_Val==DIO_PIN_HIGH)
		{
			switch(copy_Port)
			{
...
uint8 DIO_u8TogPinValue(DIO_Port_t copy_Port,DIO_Pin_t copy_PinNum)
{
	uint8 Local_u8ErrorState=OK;
	if ( (copy_PinNum<=DIO_PIN7) && (copy_PinNum>=DIO_PIN0) )
	{
